...
class AzureRotator:
    ...
    def rotate_service_principal_secret(
        self, service_principal_id: str, secret_name: Optional[str] = None, validity_days: int = 90
        ...
        try:
            ...
            password_cred = PasswordCredential(
                ...
                key_id=None,
                value=new_secret,
                custom_key_identifier=secret_name
                or f"Rotated-{datetime.utcnow().strftime('%Y%m%d')}",
            )
            ...
            if keyvault_url:
                secret_stored = self._store_in_keyvault(
                    keyvault_url, f"{sp.display_name}-secret", new_secret
                )
                if secret_stored:
    ...
    def _store_in_keyvault(self, keyvault_url: str, secret_name: str, secret_value: str) -> bool:
        ...
        try:
            ...
            secret_client.set_secret(secret_name, secret_value, content_type="text/plain")
    ...
    def rotate_storage_account_key(
        self, resource_group: str, account_name: str, key_name: str = "key1"
    ) -> Tuple[bool, Dict[str, str]]:
        ...
        try:
            ...
            storage_client.storage_accounts.regenerate_key(
                resource_group, account_name, {"key_name": key_name}
            )
            ...
            if new_key:
                ...
                keyvault_url = os.getenv("AZURE_KEYVAULT_URL")
                if keyvault_url:
                    self._store_in_keyvault(keyvault_url, f"{account_name}-{key_name}", new_key)
    ...
    def rotate_cosmos_db_key(
        self, resource_group: str, account_name: str, key_kind: str = "primary"
    ) -> Tuple[bool, Dict[str, str]]:
        ...
        try:
            ...
            cosmos_client.database_accounts.regenerate_key(
                resource_group, account_name, {"key_kind": key_kind}
            )
            ...
            new_key = (
                keys.primary_master_key if key_kind == "primary" else keys.secondary_master_key
            )
            ...
            if new_key:
                ...
                keyvault_url = os.getenv("AZURE_KEYVAULT_URL")
                if keyvault_url:
                    self._store_in_keyvault(keyvault_url, f"{account_name}-{key_kind}", new_key)
    ...
    def validate_new_credentials(self, client_id: str, client_secret: str) -> bool:
        ...
        try:
            ...
            test_credential = ClientSecretCredential(
                tenant_id=self.tenant_id, client_id=client_id, client_secret=client_secret
            )
