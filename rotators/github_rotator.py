    ...
    def revoke_personal_access_token(
        ...
        try:
            ...
            if token_id:
                ...
                result["message"] = "Token marked for revocation. Manual removal recommended via GitHub Settings."
            else:
                # Without token ID, best we can do is invalidate it in our systems
                result["status"] = "partial"
                result["message"] = "Token cannot be directly revoked via API. Remove manually from GitHub Settings > Personal Access Tokens."
    ...
    def rotate_github_app_token(
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                result["token"] = token_data["token"][:8] + "..." + token_data["token"][-8:]  # Masked
