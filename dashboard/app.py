...
def main():
    ...
    page = st.sidebar.radio(
        "Select Page", ["Dashboard", "Scan Repository", "Rotation Center", "Reports", "Settings"]
    )
...
def show_dashboard():
    ...
    if timeline_data:
        fig = px.line(
            timeline_data, x="date", y="findings", title="Findings Over Time", markers=True
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    if recent_findings:
        ...
        st.dataframe(
            df[["type", "file", "severity", "repository", "detected_at"]], use_container_width=True
        )
...
def show_scan_page():
    ...
    scan_type = st.selectbox(
        "Select Scan Type", ["Local Repository", "GitHub Repository", "GitHub Organization"]
    )
...
def scan_local_repo():
    ...
    with col1:
        ...
        max_commits = st.number_input(
            "Max commits to scan", min_value=10, max_value=10000, value=100, step=50
        )
...
def scan_github_repo():
    ...
    repo_name = st.text_input(
        "Repository Name", placeholder="owner/repository", help="Format: owner/repository"
    )
...
def scan_github_org():
    ...
    org_name = st.text_input(
        "Organization Name", placeholder="my-organization", help="GitHub organization name"
    )

    max_repos = st.number_input(
        "Maximum repositories to scan", min_value=1, max_value=500, value=50, step=10
    )
    ...
    if st.button("🚀 Start Organization Scan", type="primary"):
        ...
        with st.spinner(f"Scanning {org_name} organization..."):
            try:
                scanner = GitHubScanner(
                    token=github_token, scan_history=scan_history, scan_prs=False, max_commits=50
                )
...
def show_rotation_page():
    ...
    rotation_type = st.selectbox("Select Provider", ["AWS", "Azure", "GitHub"])
...
def rotate_aws_credentials():
    ...
    access_key_id = st.text_input(
        "Access Key ID", placeholder="AKIA...", help="AWS Access Key ID to rotate"
    )
    ...
    region = st.selectbox(
        "AWS Region", ["us-east-1", "us-west-2", "eu-west-1", "eu-central-1", "ap-southeast-1"]
    )
        ...
        with st.spinner("Rotating AWS credentials..."):
            try:
                ...
                success, details = rotator.rotate_iam_access_key(
                    access_key_id, user_name if user_name else None
                )
...
def rotate_azure_credentials():
    ...
    tenant_id = st.text_input(
        "Tenant ID", value=os.getenv("AZURE_TENANT_ID", ""), help="Azure AD Tenant ID"
    )
    ...
    validity_days = st.number_input(
        "Secret Validity (days)", min_value=30, max_value=365, value=90, step=30
    )
        ...
        with st.spinner("Rotating Azure credentials..."):
            try:
                ...
                success, details = rotator.rotate_service_principal_secret(
                    service_principal_id, validity_days=validity_days
                )
...
def rotate_github_credentials():
    ...
    rotation_type = st.selectbox(
        "Credential Type", ["Personal Access Token", "Deploy Key", "Webhook Secret"]
    )
    ...
    if rotation_type == "Personal Access Token":
        ...
        repo_name = st.text_input(
            "Repository", placeholder="owner/repository", help="Repository to rotate deploy key for"
        )
...
def show_settings_page():
    ...
    with st.expander("AWS Settings"):
        ...
        aws_profile = st.text_input(
            "AWS Profile", value=os.getenv("AWS_PROFILE", "default"), help="AWS CLI profile to use"
        )
    ...
    max_file_size = st.number_input(
        "Max File Size (MB)", min_value=1, max_value=100, value=10, help="Maximum file size to scan"
    )
