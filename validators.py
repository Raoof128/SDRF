...
def validate_aws_access_key(access_key: str) -> str:
    ...
    if not any(access_key.startswith(prefix) for prefix in valid_prefixes):
        raise ValidationError(f"Invalid AWS access key prefix. Must start with: {', '.join(valid_prefixes)}")
...
def validate_github_token(token: str) -> str:
    ...
    if not any(token.startswith(prefix) for prefix in valid_prefixes):
        raise ValidationError(f"Invalid GitHub token format. Must start with: {', '.join(valid_prefixes)}")
...
def validate_positive_int(value: Any, name: str = "value", min_value: int = 1, max_value: Optional[int] = None) -> int:
