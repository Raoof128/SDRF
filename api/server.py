...
async def scan_github_organization(
    ...
    try:
        ...
        # Perform scan
        all_findings = scanner.scan_organization(
            request.org_name,
            request.max_repos
        )
        ...
        return {
            ...
            "findings_by_repo": {
                repo: [f.to_dict() for f in findings]
                for repo, findings in all_findings.items()
            },
            "scan_time": scan_time,
            "report_path": report_path
    ...
    try:
        ...
        if request.token_type == "pat":
            ...
            if not request.installation_id:
                raise HTTPException(status_code=400, detail="Installation ID required for app token rotation")
            ...
            if not request.repo_name:
                raise HTTPException(status_code=400, detail="Repository name required for deploy key rotation")
