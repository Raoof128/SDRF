...
class RegexEngine:
                ...
                if "regex" in pattern_config:
                    try:
                        self.compiled_patterns[key] = re.compile(
                            pattern_config["regex"], re.IGNORECASE | re.MULTILINE
                        )
    ...
    def scan_file(
        self, file_path: Path, commit_info: Optional[Dict[str, str]] = None
    ) -> List[SecretFinding]:
    ...
    def _is_valid_finding(self, finding: SecretFinding, existing: List[SecretFinding]) -> bool:
        ...
        for existing_finding in existing:
            if (
                existing_finding.secret_value == finding.secret_value
                and existing_finding.file_path == finding.file_path
                and existing_finding.line_number == finding.line_number
            ):
                return False
