...
class GitHubTokenDetector(RegexEngine):
    ...
    def detect_github_tokens(self, text: str, file_path: str = "unknown") -> List[SecretFinding]:
        ...
        for finding in findings:
            ...
            if "github" not in finding.secret_type.lower() and "ssh" not in finding.secret_type.lower() and "npm" not in finding.secret_type.lower():
                continue
    ...
    def check_github_actions_secrets(self, workflow_content: str) -> List[SecretFinding]:
        ...
        for match in re.finditer(env_patterns[0], workflow_content):
            ...
            if (len(value) > 10 and 
                ...
                not value.startswith("{{") and
                any(keyword in key.lower() for keyword in ["token", "secret", "key", "password", "api"])):
    ...
    def detect_ssh_keys(self, text: str, file_path: str = "unknown") -> List[SecretFinding]:
        ...
        for match in re.finditer(ssh_key_pattern, text):
            ...
            is_github_related = any(keyword in context_before.lower() + context_after.lower() 
            ...
            findings.append(SecretFinding(
                ...
                column=match.start() - text.rfind('\n', 0, match.start()),
                severity="critical",
                description="SSH Private Key detected" + (" (likely GitHub related)" if is_github_related else ""),
                context="SSH Key",
