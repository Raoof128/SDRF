...
class GitHubScanner:
    ...
    def __init__(
        ...
        self.skip_paths = {"node_modules", ".git", "vendor", "venv", "__pycache__"}
    ...
    def scan_organization(
        self, org_name: str, max_repos: int = 50
            ...
            for detector_name, detector in self.detectors.items():
                if detector_name == "aws" and isinstance(detector, AWSDetector):
                    ...
                    findings = detector.detect_high_entropy_strings(
                        content, file_content.path, commit_info
                    )
                else:
    ...
    def _scan_commits(self, repo: Repository) -> None:
        ...
        try:
            ...
            for i, commit in enumerate(commits[: self.max_commits]):
                ...
                # Scan commit message
                self._scan_text(
                    commit.commit.message, f"commit_message_{commit.sha[:8]}", repo, commit_info
                )
                    ...
                    for file in files:
                        if file.patch:
                            ...
                            if added_lines:
                                content = "\n".join(added_lines)
                                self._scan_text(
                                    content, f"pr_{pr.number}/{file.filename}", repo, pr_info
                                )
    ...
    def _scan_text(
        self, text: str, source: str, repo: Repository, commit_info: Optional[Dict[str, str]] = None
    ) -> None:
