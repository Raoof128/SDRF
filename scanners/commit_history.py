...
class CommitHistoryScanner:
    ...
    def _scan_commit_message(self, commit: git.Commit, commit_info: Dict) -> None:
        ...
        has_suspicious_keyword = any(keyword in message.lower() for keyword in suspicious_keywords)

        if has_suspicious_keyword:
            # Scan with all detectors
            for detector_name, detector in self.detectors.items():
                findings = detector.scan_text(
                    message, f"commit_msg_{commit.hexsha[:8]}", commit_info
                )
    ...
    def _scan_diff(self, diff, commit_info: Dict) -> None:
        ...
        try:
            ...
            if removed_lines:
                removed_content = "\n".join(removed_lines)
                for detector_name, detector in self.detectors.items():
                    removed_findings = detector.scan_text(
                        removed_content, diff.a_path or "unknown", commit_info
                    )
    ...
    def _analyze_patterns(self) -> None:
        ...
        repeat_offenders = {
            author: len(findings) for author, findings in authors.items() if len(findings) > 1
        }
        ...
        hotspots = {file: len(findings) for file, findings in files.items() if len(findings) > 1}
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        stats["commits_with_secrets"] = sum(
            1 for commit_stat in self.commit_stats.values() if commit_stat["findings"] > 0
        )
