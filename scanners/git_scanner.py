...
class GitScanner:
    ...
    def _scan_file(self, file_path: Path, commit_info: Optional[Dict[str, str]] = None) -> None:
        ...
        try:
            ...
            rel_path = (
                file_path.relative_to(self.repo_path) if file_path.is_absolute() else file_path
            )
            ...
            for detector_name, detector in self.detectors.items():
                if detector_name == "aws" and isinstance(detector, AWSDetector):
                    ...
                    findings = detector.detect_high_entropy_strings(
                        content, str(rel_path), commit_info
                    )
                else:
    ...
    def _scan_commit_history(self) -> None:
        ...
        try:
            ...
            for i, commit in enumerate(commits):
                ...
                self._scan_text(commit.message, f"commit_message_{commit.hexsha[:8]}", commit_info)
    ...
    def _scan_text(
        self, text: str, source: str, commit_info: Optional[Dict[str, str]] = None
    ) -> None:
